<template >
        <NoDataCard v-if="!tickets"></NoDataCard>
        <div  v-else
        class="flex  items-center p-2  border border-gray-700 md:p-4 bg-gray-300 rounded mb-2 w-full md:justify-between " style="min-height:200px"
        ...
        >
            ...
            <div class="space-y-2 md:space-y-4 w-1/4 flex flex-col items-center justify-start">
                ...
                <router-link :to="{ name: 'app.seats', params: {ticketCode : t.ticketCode, date: selectedDate} }">
                    <button @click="setSelectedDate(selectedDate)" class="px-2 py-2 bg-green-500 rounded hover:bg-green-400 text-white md:py-1"><span
