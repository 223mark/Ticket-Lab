<tr class="hover:bg-gray-200">
    <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
        <div class="relative h-10 w-10">
            <img class="h-full w-full rounded-lg object-cover object-center"
                src="{{ $data->operator_image ? asset('img/OperatorImage/' . $data->operator_image) : asset('img/codelab.png') }}"
                alt="" />
            <span class="absolute right-0 bottom-0 h-2 w-2 rounded-full bg-green-400 ring ring-white"></span>
        </div>
        <div class="text-sm">
            <div class="font-medium text-gray-700">{{ $data->operator_name }}</div>
            <div class="font-medium text-gray-700">{{ $data->phone }}</div>
        </div>
    </th>
    ...
    <td class="px-6 py-4 ">
        <div class="relative h-10 w-12">
            <img src="{{ asset('img/BusImage/' . $data->bus_img) }}" alt=""
                class="h-full w-full rounded-lg object-cover object-center">
        </div>

    </td>
    <td class="px-6 py-4">{{ $data->driver_name }}</td>
    ...
    <td class="px-6 py-4">
        <div class="flex justify-end gap-4">
            ...
            <a x-data="{ tooltip: 'Edite' }" href={{ route('buses#edit', $data->id) }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="h-6 w-6" x-tooltip="tooltip">
                    <path stroke-linecap="round" stroke-linejoin="round"
